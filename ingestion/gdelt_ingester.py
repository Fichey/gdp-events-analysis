...
900 (× 15 years), staying well within GDELT rate limits.

NOTE: The GDELT Doc API has no mode that returns per-country breakdown
in a single request while preserving per-theme granularity.  The minimum
number of requests for 10 countries × 6 themes is therefore 60.
The only alternative that achieves a single query is Google BigQuery,
which hosts the full GDELT dataset but requires a GCP account.
"""
import logging
import time
...
def fetch_timeline_range(
    ...
    """
    Fetch weekly article-count timeline from GDELT for one (country, theme) pair
    covering the full year range.  Returns a list of
    {"date": "YYYYMMDDHHMMSS", "value": float} dicts, or [] on permanent failure.

    Valid Doc API modes: ArtList, TimelineVol, TimelineVolNorm, TimelineVolRaw,
    TimelineTone, TimelineToneLang, TimelineLang, TimelineLangRaw.
    There is no per-country breakdown mode — hence one request per country.
    """
    params = {
...
def _parse_year_month(date_str: str) -> Optional[tuple]:
    """'YYYYMMDDHHMMSS' → (year, month), or None if too short."""
    if len(date_str) >= 6:
        return int(date_str[:4]), int(date_str[4:6])
    return None


def ingest_gdelt(db: Session, year_start: int = 2010, year_end: int = 2025) -> int:
