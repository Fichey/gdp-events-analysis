...
from gdelt_ingester import fetch_timeline_range, _parse_year_month, GDELT_COUNTRY_MAP, THEME_QUERY_MAP


class TestParseYearMonth:
    def test_parses_valid_date(self):
        assert _parse_year_month("20200315120000") == (2020, 3)

    def test_returns_none_for_short_string(self):
        assert _parse_year_month("2020") is None

    def test_parses_month_12(self):
        assert _parse_year_month("20201201000000") == (2020, 12)

    def test_parses_month_01(self):
        assert _parse_year_month("20200101000000") == (2020, 1)


class TestFetchTimelineRange:
    def test_returns_empty_on_http_error(self):
        import requests as _requests
        with patch("gdelt_ingester.cf.get") as mock_get, \
             patch("gdelt_ingester.time.sleep"):          # suppress retry delays
            mock_get.side_effect = _requests.RequestException("timeout")
            result = fetch_timeline_range("US", "PROTEST", 2020, 2022)
        assert result == []
